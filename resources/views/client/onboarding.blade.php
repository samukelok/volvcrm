...
@section('content')
    <div
        class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-purple-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-2xl w-full space-y-8">
            <!-- Logo and Header -->
            <div class="text-center">
                <div
                    class="mx-auto w-16 h-16 bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl flex items-center justify-center mb-6">
                ...
                <form method="POST" action="{{ route('client.onboarding.store') }}" class="space-y-6">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-500">This will be displayed on your funnels and communications
                            </p>
                        </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <!-- Primary Color -->
                        <div>
                            <label for="primary_color" class="block text-sm font-medium text-gray-700 mb-2">Primary Brand
                                Color</label>
                            <div class="flex items-center space-x-4">
                                <input type="color" id="primary_color"
                                    class="h-12 w-20 border border-gray-300 rounded-xl cursor-pointer" value="#4e73df">
                                <input type="text" id="primary_color_text"
                                    class="flex-1 px-4 py-3 rounded-xl border border-gray-200" value="#4e73df"
                                    placeholder="#4e73df">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="flex items-center space-x-4">
                                <input type="color" id="secondary_color"
                                    class="h-12 w-20 border border-gray-300 rounded-xl cursor-pointer" value="#ffffff">
                                <input type="text" id="secondary_color_text"
                                    class="flex-1 px-4 py-3 rounded-xl border border-gray-200" value="#ffffff"
                                    placeholder="#ffffff">
                            </div>
                        </div>

                        <!-- Hidden Combined Input -->
                        <input type="hidden" name="branding" id="branding" value="#4e73df, #ffffff">

                        <p class="mt-1 text-xs text-gray-500">These colors will be used for buttons, links, and background
                            accents in your funnels</p>
                    </div>
            ...
            <!-- What's Next Section -->
            <div
                class="bg-gradient-to-r from-blue-50/80 to-purple-50/80 backdrop-blur-xl border border-white/20 rounded-2xl p-6">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const colorPickerPrimary = document.getElementById('primary_color');
        const textInputPrimary = document.getElementById('primary_color_text');

        const colorPickerSecondary = document.getElementById('secondary_color');
        const textInputSecondary = document.getElementById('secondary_color_text');

        const brandingInput = document.getElementById('branding');

        // Function to update combined hidden branding input
        function updateBrandingValue() {
            const primary = textInputPrimary.value.trim();
            const secondary = textInputSecondary.value.trim();
            brandingInput.value = `${primary}, ${secondary}`;
        }

        // Primary color syncing
        colorPickerPrimary.addEventListener('input', function () {
            textInputPrimary.value = this.value;
            updateBrandingValue();
        });

        textInputPrimary.addEventListener('input', function () {
            if (this.value.match(/^#[0-9A-F]{6}$/i)) {
                colorPickerPrimary.value = this.value;
                updateBrandingValue();
            }
        });

        // Secondary color syncing
        colorPickerSecondary.addEventListener('input', function () {
            textInputSecondary.value = this.value;
            updateBrandingValue();
        });

        textInputSecondary.addEventListener('input', function () {
            if (this.value.match(/^#[0-9A-F]{6}$/i)) {
                colorPickerSecondary.value = this.value;
                updateBrandingValue();
            }
        });

        // Initialize values on page load
        textInputPrimary.value = colorPickerPrimary.value;
        textInputSecondary.value = colorPickerSecondary.value;
        updateBrandingValue();
    });
</script>
